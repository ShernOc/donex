...
export const CharityProvider = ({ children }) => {
  ...
  const loginCharity = async (email, password) => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:5001/charities/login", {
        method: "POST",
      ...
      // get charity 
      const charityResponse = await fetch("http://127.0.0.1:5001/current_charities", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const fetchCharityById = async (charity_id) => {
    try {
      const response = await fetch(`http://127.0.0.1:5001/charities/${charity_id}`)
