...
# Route to get all charities
@target_bp.route("/all_charities", methods=["GET"])
def get_charities():
    charities = Charity.query.all()
    return jsonify({
        "charities": [
...
def update_charity_status(charity_id):
    data = request.json
    charity = Charity.query.get_or_404(charity_id)
    if "status" in data and isinstance(data["status"], bool):  # Check for boolean value
...
# Updated Route to get only approved charities
@target_bp.route("/charities/approved", methods=["GET"])
def get_approved_charities():
    status = request.args.get("status")
    if status=="approved":
